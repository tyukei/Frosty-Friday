...
-- 5. クエリ履歴とアクセス履歴を結合して、アクセスしたロールを特定する
potential_leaks as (
    select 
        accessed_tables.*, 
    ...
    from 
        ...
        on query_history.query_id = accessed_tables.query_id
),

-- 6. タグ情報を追加して最終結果を取得する
add_tag_info as (
    select 
        object_ids.tag_name,
...
from 
    add_tag_info
group by 
    tag_name, tag_value, table_name, role_name
;
